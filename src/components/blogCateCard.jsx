...
const BlogCateCard = ({ data }) => {
    return (
        <>
            <div className="col-6">
                <div className='div6'>
                    {/* <span className="mt-4 ms-3  bg-danger text-white rounded-2 p-1">
                        {data?.cate_name}
                    </span> */}
                    <img
                    ...
                    <div className='mb-0 d-flex justify-content-between align-items-center p-3'>
                        <span className=" text-secondary mt-3 butt">{data?.date}</span>
                        <span className="mt-3">
                            <Link to={`${Route_Path.BLOG}/${Route_Path.B1}/${data?.slug}`} className='btn btn-outline-danger butt'>read more</Link>
                        </span>
                    </div>
                </div>
            </div>
